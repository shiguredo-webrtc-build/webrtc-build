  ...
  if (encoder_config_.video_stream_factory) {
    ...
    rtc::scoped_refptr<VideoEncoderConfig::VideoStreamFactoryInterface>
        factory = rtc::make_ref_counted<cricket::EncoderStreamFactory>(
            encoder_config_.video_formats[0].name, encoder_config_.max_qps[0],
...
EncodedImage VideoStreamEncoder::AugmentEncodedImage(
  ...
  //RTC_LOG(LS_ERROR) << "spatial_index=" << encoded_image.SpatialIndex().value_or(-1);
  //RTC_LOG(LS_ERROR) << "simulcast_index=" << encoded_image.SimulcastIndex().value_or(-1);
  //int stream_idx = encoded_image.SpatialIndex().value_or(
  //    encoded_image.SimulcastIndex().value_or(0));
  // TODO(melpon): ここを変えるのは本来おかしいのだけど、なぜか stream_idx が 0,1,2 ではなく 0,0,2 という値になって、
  // FrameEncodeMetadataWriter に警告を出されてしまう
