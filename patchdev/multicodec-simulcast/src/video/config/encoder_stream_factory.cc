...
std::vector<webrtc::VideoStream> EncoderStreamFactory::CreateEncoderStreams(
  ...
  const absl::optional<webrtc::DataRate> experimental_min_bitrate =
      GetExperimentalMinVideoBitrate(encoder_config.codec_types[0]);
...
EncoderStreamFactory::CreateSimulcastOrConferenceModeScreenshareStreams(
  ...
  const size_t min_layers = is_multicodec ? encoder_config.number_of_streams : FindRequiredActiveLayers(encoder_config);
  layers = GetSimulcastConfig(min_layers,
