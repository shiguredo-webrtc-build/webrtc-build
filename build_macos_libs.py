...
"""WebRTC macOS FAT libraries build script.
Each architecture is compiled separately before being merged together.
By default, the library is created in out_macos_libs/. (Change with -o.)
"""
import argparse
...
SDK_OUTPUT_DIR = os.path.join(SRC_DIR, 'out_macos_libs')
...
def BuildWebRTC(output_dir, target_arch, flavor, gn_target_name,
                mac_deployment_target, libvpx_build_vp9, use_bitcode, use_goma,
                extra_gn_args):
    ...
    gn_args.append('mac_deployment_target="%s"' % mac_deployment_target)
...
def main():
    ...
    for arch in architectures:
        BuildWebRTC(args.output_dir, arch, args.build_config, gn_target_name,
                    MAC_DEPLOYMENT_TARGET, LIBVPX_BUILD_VP9, args.bitcode,
