...
- `setTlsClientCertificateChain(String privateKeyPem, String certificateChainPem)`

単一の証明書用と証明書チェーン用 API は分けて明示的に指定する。
内部では libwebrtc 本体の API に合わせて、前者は
`SSLIdentity::CreateFromPEMStrings()`、後者は
`SSLIdentity::CreateFromPEMChainStrings()` を使って `tls_client_identity` を生成する。
