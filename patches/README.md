...
Android 向け `webrtc.jar` と `libwebrtc.aar` 内の `classes.jar` には次のクラスが含まれる。
...
パッチは `android` と `android_sdk` に適用する。
`android` では `webrtc.android.tar.gz` 内の `jar/webrtc.jar`、`android_sdk` では `webrtc.android_sdk.tar.gz` 内の `aar/libwebrtc.aar` に反映する。
...
python3 run.py build android_sdk
python3 run.py package android_sdk

work_dir="$(mktemp -d)"
tar -xOf _package/android/webrtc.android.tar.gz webrtc/jar/webrtc.jar > "$work_dir/webrtc.jar"
tar -xOf _package/android_sdk/webrtc.android_sdk.tar.gz webrtc/aar/libwebrtc.aar > "$work_dir/libwebrtc.aar"
unzip -p "$work_dir/libwebrtc.aar" classes.jar > "$work_dir/classes.jar"

for jar in "$work_dir/webrtc.jar" "$work_dir/classes.jar"; do
  unzip -Z1 "$jar" | grep -E '^(org/jni_zero/(JniZero(\$Natives)?|JniZeroJni|GEN_JNI)|J/N)\.class$'
  javap -classpath "$jar" org.jni_zero.GEN_JNI | grep 'org_jni_1zero_JniZero_setJniClassLoader'
  javap -classpath "$jar" -c org.jni_zero.GEN_JNI | grep 'J/N'
done

rm -rf "$work_dir"
```

取り出した 2 つの jar に上記 5 クラスが含まれ、`GEN_JNI` が `J/N` を呼び出すことを確認する。
