...
WebRTC M149 以降の JNI Zero 構成に追従し、Android 向け Java 成果物へ実行時に必要な生成クラスを追加するパッチ。

WebRTC M149 では、`org.jni_zero.JniZero.setJniClassLoader()` が `JniZeroJni.get().setJniClassLoader()` を呼び出すようになった。
しかし、変更前の Android 向け Java 成果物には `JniZeroJni.class` が含まれていないため、R8 が参照先を解析すると Missing class エラーになる。

`JniZeroJni.class` だけを追加しても、実行時に必要なクラスは揃わない。
`generate_jni` が作る `GEN_JNI` は Java のコンパイル用であり、最終的な native library のハッシュ化 JNI symbol には対応していない。
このパッチは、Java と native の依存関係から最終的な JNI registration の Java クラスを生成し、`JniZeroJni.class` とともに配布用 jar へ追加する。
