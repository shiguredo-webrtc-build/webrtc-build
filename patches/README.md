...
`PeerConnection.IceServer.Builder` に
`setTlsClientCertificate(String privateKeyPem, String certificatePem)` を追加し、
JNI 側で 2 つの PEM 文字列から `SSLIdentity` を生成して
`tls_client_identity` に設定する。

証明書 PEM に複数の証明書が含まれる場合は
`SSLIdentity::CreateFromPEMChainStrings()` を使い、
単一証明書の場合は `SSLIdentity::CreateFromPEMStrings()` を使う。
