...
armv8 ターゲットはクロスコンパイルのためホスト OS バージョンに依存しない。`ubuntu-26.04_armv8` も `ubuntu-26.04` runner 上で multistrap により rootfs を取得してクロスビルドする。
...
- `add_path` 関数のデフォルト値を `is_after=True` に変更 (run.py:87)
  - depot_tools 同梱の ninja ラッパーが CIPD 経由で本体をダウンロードしようとするが、26.04 では CIPD が未対応のため失敗する
  - `is_after=True` により depot_tools は PATH 末尾に追加され、システム ninja (`/usr/bin/ninja`, apt の `ninja-build` パッケージ) が優先される
  - depot_tools 専用ツール (`gn`, `gclient`, `fetch`) はシステムに存在しないため、PATH 末尾でも `shutil.which` で解決される (フォールバック)

### `.github/workflows/build.yml` の修正

- `build-linux` の `matrix.platform` に以下を追加:
  - `- name: ubuntu-26.04_armv8` / `runs-on: ubuntu-26.04`
