...
0016 を重ねた追試では、iOS 26.2 でも入力を開くテストに同じ RPC タイムアウトが再現した。
Simulator を再起動した後の単独実行でも再現したため、特定のランタイムだけの問題とは断定しない。
前述の 12 件の成功結果はあるが、音声入力初期化が再実行時に不安定になる原因と対処は未確定であり、クラッシュを修正済みとは扱わない。
